...
class GeminiAudioSession:
    ...
    def _build_config(self) -> types.LiveConnectConfig:
        ...
        system_text = self.tenant.system_instructions + "\n\n" + coffee_state.build_system_instructions()
    ...
    async def _handle_tool_call(self, tool_call, session) -> None:
        """Handle a function-call request coming from Gemini (e.g. pour_coffee)."""
        function_responses = []
...
class CoffeeState:
    """Tracks the coffee-pour signal shown on the standalone /coffee display page.

    The value is 0 by default and becomes 1 for COFFEE_POUR_DURATION_SECONDS
    whenever the bot is asked (via voice) to pour coffee, using one of the
    configurable trigger phrases below.
    """
    ...
    def _load_phrases(self) -> list[str]:
        if self.PHRASES_FILE.exists():
            try:
                data = json.loads(self.PHRASES_FILE.read_text(encoding="utf-8"))
                if isinstance(data, list) and data and all(isinstance(p, str) and p.strip() for p in data):
                    return [p.strip() for p in data]
            except Exception as e:
                logger.error(f"Failed to load coffee phrases: {e}")
        return list(self.DEFAULT_PHRASES)

    def _save_phrases(self) -> None:
        try:
            self.PHRASES_FILE.write_text(
                json.dumps(self.phrases, ensure_ascii=False, indent=2), encoding="utf-8"
            )
        except Exception as e:
            logger.error(f"Failed to save coffee phrases: {e}")

    def set_phrases(self, phrases: list[str]) -> None:
        """Replace the list of trigger phrases (falls back to the default if empty)."""
        cleaned = [p.strip() for p in phrases if p.strip()]
        self.phrases = cleaned or list(self.DEFAULT_PHRASES)
        self._save_phrases()
    ...
    @property
    def value(self) -> int:
        """Current signal value: 1 while active, 0 otherwise."""
        return 1 if time.time() < self.active_until else 0

    def build_tool(self) -> "types.Tool":
        """Build the Gemini function-calling tool declaration for pouring coffee."""
        phrase_list = "، ".join(f'"{p}"' for p in self.phrases)
        return types.Tool(function_declarations=[
            types.FunctionDeclaration(
                name="pour_coffee",
                description=(
                    "استدعِ هذه الأداة فوراً عندما يطلب المستخدم منك صب القهوة، سواء استخدم إحدى هذه "
                    f"العبارات بالضبط أو عبارة أخرى بنفس المعنى: {phrase_list}. "
                    "هذه الأداة تُشغّل آلة صب القهوة الحقيقية فعلياً."
                ),
            )
        ])

    def build_system_instructions(self) -> str:
        """Extra system-prompt text (appended dynamically) describing when to use the tool."""
        phrase_list = "، ".join(f'"{p}"' for p in self.phrases)
        return (
            "### أداة صب القهوة\n"
            f"إذا طلب منك المستخدم صب القهوة بأي صيغة قريبة من هذه العبارات: {phrase_list}، "
            "يجب عليك استدعاء الأداة pour_coffee فوراً، ثم تخبره أن القهوة تُصب الآن."
        )


coffee_state = CoffeeState()
...
class CoffeePhrasesRequest(BaseModel):
    """Request model for updating the coffee trigger phrases."""
    phrases: list[str] = Field(..., min_length=1, max_length=50)


class CoffeePhrasesResponse(BaseModel):
    """Response model listing the current coffee trigger phrases."""
    phrases: list[str]


class CoffeeStatusResponse(BaseModel):
    """Response model for the public coffee display page."""
    value: int
...
@app.get("/admin/coffee-phrases", response_model=CoffeePhrasesResponse)
async def admin_get_coffee_phrases() -> CoffeePhrasesResponse:
    """Get the current list of coffee-pour trigger phrases."""
    return CoffeePhrasesResponse(phrases=coffee_state.phrases)


@app.post("/admin/coffee-phrases", response_model=AdminResponse)
async def admin_set_coffee_phrases(request: CoffeePhrasesRequest) -> AdminResponse:
    """Update the list of coffee-pour trigger phrases and restart sessions to apply it."""
    coffee_state.set_phrases(request.phrases)
    await connection_manager.broadcast({"type": "restart"})
    return AdminResponse(
        message=f"تم حفظ {len(coffee_state.phrases)} عبارة، جاري إعادة تشغيل {connection_manager.connection_count} جلسة"
    )


@app.post("/admin/coffee-trigger", response_model=AdminResponse)
...
async def coffee_status() -> CoffeeStatusResponse:
    """Return the current coffee signal (0 or 1). Polled by the standalone /coffee page."""
    return CoffeeStatusResponse(value=coffee_state.value)
